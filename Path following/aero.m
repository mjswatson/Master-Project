...
a=interp1(input.h_stand,input.a_stand,h,'linear','extrap');
M=speed/a;
...
%find air conditions
Temp=interp1(input.h_stand,input.temp_stand,h,'linear','extrap')-273.15;
dens=interp1(input.h_stand,input.dens_stand,h,'linear','extrap');
if dens<0
    dens=0;
end
...
if M<=0.9
    [CD,Cl]=subsonic(alpha,M,Cf,input);
    x_cp=input.Ln-(input.Vn/input.Abn);
elseif M<=4
