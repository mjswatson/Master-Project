...
dsdt=V*iTBI;
if final_state(1)<0
x_check=abs(final_state(1)*1.1)+x(1);    
else
x_check=final_state(1)*1.1-x(1);
end
if final_state(2)<0
 z_check=abs(final_state(2)*1.1)+x(2);    
else
z_check=final_state(2)*1.1-x(2);
end
y_check=final_state(3)*1.1-x(3);
