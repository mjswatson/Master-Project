...
max_step_time=15; %inital if path cant be followed at this interval will be increased - give more freedom
increase_factor=1.2; %factor of time increase
max_search=50000; %number of random control variation that will be tried before abandoning the path - increases with search factor as well
init_tol=1.5;
con_tol=1e-3;        %fmincon contraint tolerance
step_tol=1e-3;      %fmincon smallest step size
objective_limit=1e-2;
N=3;       %number of turning points
M=10;       %nodes per line
...
auxdata.max_rotation=max_rotation;
   ...
   quat=quats(:,:,i);
    ...
    while j<numel(line(:,1))
        ...
        final_state=[line(j+1,:),quat(j+1,:)];
         ...
         if plot_corner
%              corner_quat=zeros(10,4);
%              while corner_quat(end,:)~=quat(k,:)
        [points]=circles_for_days(quat,j,x_init,a,line);
% %              end
...
%         %remove excess data back to before turn begins
        ...
        for q=1:t_end
            point_checker=abs(points(1,1:3))-abs(flight(q,1:3));
            if point_checker<0
                break
            end
        end
...
%             if j~=1 %causing issues with convergence
%              while dist>norm(x_init(4:6))*max_step_time*4 %if further than twice distance covered at current speed
%              x_new=linspace(x_init(1),final_state(1),4);
%              z_new=linspace(x_init(2),final_state(2),4);
%              y_new=linspace(x_init(3),final_state(3),4);
%              quat_new1=linspace(x_init(11),final_state(4),4);
%              quat_new2=linspace(x_init(12),final_state(5),4);
%              quat_new3=linspace(x_init(13),final_state(6),4);
%              quat_new4=linspace(x_init(14),final_state(7),4);
%              final_state=[x_new(2) z_new(2) y_new(2) quat_new1 quat_new2 quat_new3 quat_new4];
%              dist=norm(final_state(1:3)-x_init(1:3));
%             end
%             end
        check2=0;
        while check2~=1
        [t,no_control_points]=t_finder(auxdata,x_init,input,final_state,dist,plot_corner);
        ...
        elseif t>max_step_time 
           x_new=linspace(x_init(1),final_state(1),3);
            ...
            if and(plot_corner,abs(final_state(1:3))<abs(points(4:6)))
                final_state(1:3)=points(4:6);
            end
            dist=norm(final_state(1:3)-x_init(1:3));
        else
            check2=1;
        ...
        if and(init_tol>error,error>-init_tol)
            ...
            if iter>(max_search/5000) %stop if too many iterations run and no solution found
                break
            end
        end
        %% optimise results within met criteria 
         if iter>(max_search/5000)
             break
         end
%         [row,col]=size(xin);
...
%         ui=ui(:);
        ...
        options=optimoptions(@fmincon,'UseParallel',true,'Display','Iter','MaxFunctionEvaluations',Inf,'ObjectiveLimit',objective_limit,'StepTolerance',1e-9);
        ...
        [test,time]=state_dot_full_test(x_init,u,t_step,input,t,N,M,aero_control,Nstates,final_state,T,dimensions,Ncontrols);
        %% check fmincon has hit target or on same line- if not tries again
        check=error_calc(test,final_state,line,j,max_rotation); %check how close to target
        %% ensure time step enough
        dist_store(iter2)=norm(abs(test(end,1:3))-abs(x_init(1:3)));
        error_store(iter2)=check;
        if and(iter2>7,or(max(dist_store)<dist,min(error_store)<0.3)) %if tried 10 times and all fallen short by half or more double the time
            t=ceil(t*(dist/mean(dist_store)));
            if t>max_step_time   %ensures stays within max
                t=max_step_time;
            end
            if and(max(dist_store)<dist,t==max_step_time)
                if not(plot_corner)
            x_new=linspace(x_init(1),final_state(1),3);
            z_new=linspace(x_init(2),final_state(2),3);
            y_new=linspace(x_init(3),final_state(3),3);
     ...
     u_all(t_start:t_end,:)=u(1:numel(time)-1,:);
