function dxdt=odefunc(t,x,u,t_step,input,T,N,aero_control,interp,dimensions)
S=[x(1) x(2) x(3)];
V=[x(4) x(5) x(6)];
w=[x(7) x(8) x(9)];
mf=x(10);
...
Fa=Fa*(aero_control/100);
Ma=Ma*(aero_control/100);
else
    Fa=[0 0 0];
    Ma=[0 0 0];
end
...
dwdt=[dwdt(3),dwdt(2),dwdt(1)];
mag_dwdt=sqrt(dwdt(1)^2+dwdt(2)^2+dwdt(3)^2);
[pitch,roll,yaw]=quat2angle(q,'YZX');
pitch=rad2deg(pitch);
yaw=rad2deg(yaw);
dqdt=0.5*wcross*q(:);
