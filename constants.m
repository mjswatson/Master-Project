...
quat=angle2quat(pitch,roll,yaw,'YZX');
...
pressure=griddedInterpolant(h_stand,press_stand,'linear','linear');
temp=griddedInterpolant(h_stand,temp_stand,'linear','linear');
dens=griddedInterpolant(h_stand,dens_stand,'linear','linear');
a=griddedInterpolant(h_stand,a_stand,'linear','linear');
% aero interps
[cdcx,cdcV]=Cdc_interp;
Cdc=griddedInterpolant(cdcx,cdcV,'spline','spline');
[cdbx,cdbv]=Cdb_interp;
Cdb=griddedInterpolant(cdbx,cdbv,'spline','spline');
[theta_dat,alpha_dat,K_data]=K_theta_interp;
 K_theta=griddedInterpolant({alpha_dat,theta_dat},K_data,'spline');
