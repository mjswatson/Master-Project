...
start_u=load('path2.mat');  %file directory if not using random start points
     ...
     starts=load('random7_2D_short.mat');
     Iter=numel(starts.constraint);
    end
end
save_name='random7_2D_short2.mat'; %file results will be saved in
max_time=1; %max time in hours
posf=[-8000 1 100000];
plot_iter=false;
...
yi=[x;u];
    ...
    for i=1:Iter
        ...
        strtpt=transpose(starts.y_opt_store);
    end
end
...
if time>40*3600
    ...
    y_opt_store=transpose(strtpt);
end
time2=toc(overall);
if time2+3600<max_time
    time_left=max_time-time2;
    time_per_it2=time_left/Iter;
    i=1;
    for i=1:Iter
  options=optimoptions(@patternsearch,'UseParallel',true,'MaxFunctionEvaluations',Inf,'StepTolerance',1e-5,'Display','Iter','MaxIter',max_iter,'UseCompletePoll',true,'MeshTolerance',1e-6,'MaxTime',time_per_it2,'Cache','on');%'SearchFcn','MADSPositiveBasis2N');  
y0=y_opt_store(:,i);      
 [y_opt,f,exitflag,output]=patternsearch(@(y)objfun(y, auxdata,input),y0,[],[],[],[],yL,yU,@(y) confun(y, auxdata,input,time_gaps),options);
y_opt_store(:,i)=y_opt;
 x=zeros(N,Nstates);
x(2:end,:) = reshape(y_opt(1:Nstates*(N-1)), [], Nstates);
x=x.*normaliser;
x(1,:)=x0;
if run_fmincon
x_opt_store(:,:,i+Iter)=x;
u = reshape(y_opt(Nstates*(N-1)+1: end), [], Ncontrols);
u_store(:,:,i+Iter)=u;
y_opt_store(:,i+Iter)=y_opt;
f_store(i+Iter)=f;
constraint(i+Iter)=output.maxconstraint;
output_store{i+Iter}=output;
else
 x_opt_store(:,:,i)=x;
u = reshape(y_opt(Nstates*(N-1)+1: end), [], Ncontrols);
u_store(:,:,i)=u;
f_store(i)=f;
constraint(i)=output.maxconstraint;
output_store{i}=output; 
y_opt_store(:,:,i)=y_opt;
end
    end
